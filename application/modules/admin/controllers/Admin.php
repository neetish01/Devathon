...
function send_password($special)
{
	...
	{              
		...
		$subject = "TAPS NITW Account Activation";
		$message = "Welcome to Training and Placement(TAPS) portal.\nYour username is your email address: ".$reset['identity']." \nTo activate and reset password click link below <br> ".base_url('auth/reset_password/'.$reset['forgotten_password_code']);
...
function create_user_stat()
{
	...
	//print_r($data);
	$dupli=$this->admin_model->create_single_user($data);
	if($dupli==null)
	{
		$data['status']="User Created";
		$data['email_disable']=0;
	}
	else
	{
		$data['status']="duplicate user";
		$data['email_disable']=1;
	}
	$this->render_page("user_created",$data);
}
