...
class Admin_model extends CI_Model
{
	...
	{    
		...
		foreach($array as $row)
		{
			...
			// echo "User ID array:";
				...
				$user_academics['userid']=$user_id['id'];

				$this->db->insert('users_data',$user_data);
				$this->db->insert('gpa',$user_cgpa);
				$this->db->insert('academics',$user_academics);
			}
	...
	{    
		...
		$data['phone']=$array['first_name'];
		$data['username']='taps'.$array['roll_number'];
		$identity=$array['email'];
		$data['active']=1;

		$email=$array['email'];
		$data['email']=$array['email'];
		$data['roll_number']=$array['roll_number'];
		$data['registration_number']=$array['registration_number'];
		$rand=random_string('alnum', 8);
		...
		else
		{
			...
			$user_data['userid']=$user_id['id'];
			$user_data['name'] = $array['first_name']." ".$array['last_name'];
			$user_data['roll_number']=$array['roll_number'];
			$user_data['registration_number']=$array['registration_number'];
			$user_data['first_name ']=$array['first_name'];
			$user_data['last_name ']=$array['last_name'];
			$user_data['specialization_id']=$array['specialization_id'];
			$user_cgpa['userid']=$user_id['id'];
			$user_cgpa['approval_status']=0;
			$user_data['encrypt']=$array['roll_number'].$rand;
			$user_academics['userid']=$user_id['id'];
			$this->db->insert('users_data',$user_data);
			$this->db->insert('gpa',$user_cgpa);
			$this->db->insert('academics',$user_academics);
