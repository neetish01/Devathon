	...
	function forgot_password()
	{
		...
		else
		{
			...
			if ($forgotten)
			{
				...
				$subject = "test";
				$message = "your username is  your email:".$forgotten['identity'].". To reset password click link below  ".base_url('auth/reset_password/'.$forgotten['forgotten_password_code']);

		$to = $forgotten['identity'];//Get credentials from TAPS Authorities
	...
	if (isset($_POST) && !empty($_POST))
	{
		...
		if ($this->form_validation->run() === TRUE)
		{
			...
			if($this->ion_auth->update($user->id, $data))
			{
				...
				if ($this->ion_auth->is_admin())
				{
					redirect('auth/manageuser', 'refresh');
				}
				else
				{
					redirect('/', 'refresh');
