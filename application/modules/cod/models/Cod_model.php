...
class Cod_model extends CI_Model
{
	...
	function report($data)
	{
		...
		$this->db->join('eligibility as ec','ec.visit_id = company_visit.id','left');
		$this->db->join('eligibility','eligibility.specialization_id','users_data.specialization_id','left');
	...
	function get_application_data($data=null,$status=null)
	{
		...
		{
			$id=$this->ion_auth->get_user_id();
			$branch=$this->db->select('specialization_id')
			->from('users_data')
			->where('users_data.userid',$id);
			$query=$this->db->get();
			$qes=$query->result_array();
			
			$this->db->select('*')
			->from('users_data')
			->join('users', 'users_data.userid = users.id','left')
			->join('gpa', 'users_data.userid = gpa.userid','left')
			->where('users_data.specialization_id',$qes[0]['specialization_id']);
			$query=$this->db->get();
			$res=$query->result_array();
			return $res;
