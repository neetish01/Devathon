...
function get_cod()
{
	$q="SELECT users_data.first_name,users_data.last_name,users_data.mobile,users.email,specialization.name FROM users_groups 
	LEFT JOIN users_data on users_data.userid=users_groups.user_id
	LEFT JOIN users on users.id=users_data.userid
	LEFT JOIN specialization on specialization.id=users_data.specialization_id
	LEFT JOIN department on specialization.department_id=department.id
...
{
	$id=$this->ion_auth->get_user_id();
	$q="SELECT users.last_login,users.email,users_data.deactivate_status,users.ip_address FROM users 
	LEFT JOIN users_data on users_data.userid=users.id
	WHERE users.id=$id";
	$res=$this->db->query($q)->result_array();
	return $res[0];
}
